const map = L.map('map', { minZoom: 1, zoomSnap: 0.25, maxZoom: 17, zoomControl: false, worldCopyJump: true }).setView([30, -25], 2);
...
function fitAreas() {
  if (visibleAreas.length) map.fitBounds(visibleAreas.map(a => [a.lat,a.lng]), { padding: [55,55], maxZoom: 10 });
}
function renderPins() {
  ...
  clusters.forEach(cluster => {
    ...
    const title = combined ? `${cluster.count} visited places across ${cluster.areas.length} cities. Zoom in.` : `${cluster.areas[0].name}: ${cluster.count} visited ${cluster.count === 1 ? 'place' : 'places'}`;
...
function filterPlaces() {
  ...
  visibleAreas = areas.map(area => ({ ...area, places: area.places.filter(p => `${p.name} ${p.city} ${area.name}`.toLocaleLowerCase().includes(query)) })).filter(a => a.places.length);
  const count = visibleAreas.reduce((total,a) => total+a.places.length,0);
  document.getElementById('map-stats').textContent = `${count.toLocaleString()} visited ${count === 1 ? 'place' : 'places'} · ${visibleAreas.length} city ${visibleAreas.length === 1 ? 'area' : 'areas'} · From Yelp`;
...
document.getElementById('show-all').addEventListener('click', () => { clearTimeout(searchTimer); search.value=''; filterPlaces(); fitAreas(); });
